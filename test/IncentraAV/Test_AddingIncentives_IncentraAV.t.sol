...
contract Test_AddingIncentives_IncentraAV is IncentraTestBase {
    function setUp() external {
        setupIncentraBaseEnvironment();
        deployIncentraImplementations();
    }

    function test_RevertIf_AddingIncentives_IncentraAV(
        address _ip,
        ...
        prankModifier(_ip)
    {
        ...
        vm.expectRevert(IncentraAV.AddingIncentivesNotSupported.selector);
        incentiveLocker.addIncentives(incentiveCampaignId, incentivesOffered, incentiveAmountsOffered, new bytes(0));
    }
}
